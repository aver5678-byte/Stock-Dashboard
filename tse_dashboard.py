...
def page_downward_bias():
    ...
    def get_bar_html(title, m_data, theme_color="#38BDF8"):
        ...
        p2 = 100 - p1
        return f'<div style="background:rgba(255,255,255,0.03); padding:25px 30px; border-radius:12px; border:1px solid rgba(255,255,255,0.08); margin-bottom:20px;"><div style="display:flex; justify-content:space-between; align-items:end; margin-bottom:15px;"><div style="font-size:22px; color:{theme_color}; font-weight:950; letter-spacing:1px;">{title} <span style="font-size:16px; color:#94A3B8; font-weight:500;">(樣本數 {count} 次)</span></div><div style="font-size:18px; color:#F1F5F9; font-family:\'JetBrains Mono\'; font-weight:800;">平均總耗時 <span style="color:{theme_color}; font-size:24px;">{total:.1f}</span> 天</div></div><div style="height:45px; background:#0F172A; border-radius:8px; overflow:hidden; border:2px solid #334155; display:flex; box-shadow:inset 0 2px 10px rgba(0,0,0,0.5);"><div style="width:{p1}%; height:100%; background:linear-gradient(90deg, #F59E0B, #EF4444); display:flex; align-items:center; justify-content:center; color:white; font-size:16px; font-weight:950; text-shadow:0 1px 3px rgba(0,0,0,0.5); border-right:1px solid rgba(0,0,0,0.2); transition: width 0.5s ease;">觸發➔落底({t_bt:.1f}d)</div><div style="width:{p2}%; height:100%; background:linear-gradient(90deg, #3B82F6, #10B981); display:flex; align-items:center; justify-content:center; color:white; font-size:16px; font-weight:950; text-shadow:0 1px 3px rgba(0,0,0,0.5); transition: width 0.5s ease;">落底➔解套({bt_r:.1f}d)</div></div></div>'

    hud_html = f'<div style="background:#0F172A; border:4px solid #334155; border-radius:16px; padding:50px; margin-bottom:50px; box-shadow:0 30px 60px rgba(0,0,0,0.6);"><div style="display:flex; flex-direction:column; gap:40px;"><div style="display:flex; justify-content:space-between; align-items:stretch; gap:50px;"><div style="flex:1; display:flex; flex-direction:column; align-items:center; justify-content:center; text-align:center; border-right:2px solid rgba(255,255,255,0.1); padding-right:40px;"><div style="font-size:20px; color:#94A3B8; font-weight:800; margin-bottom:20px; display:flex; align-items:center; gap:12px;"><span style="width:12px; height:12px; background:{score_color}; border-radius:50%; box-shadow:0 0 15px {score_color};"></span>目前距離前高跌幅</div><div style="display:flex; flex-direction:column; gap:20px; align-items:center;"><div style="font-family:\'JetBrains Mono\'; font-size:92px; font-weight:950; color:{score_color}; line-height:0.9; letter-spacing:-5px; text-shadow:0 0 30px {score_color}44;">{current_dd:.1f}%</div><div style="display:flex; align-items:center; gap:20px;"><div style="font-family:\'JetBrains Mono\'; font-size:20px; font-weight:900; color:#EF4444; background:rgba(239, 68, 68, 0.15); padding:8px 20px; border-radius:8px; border:2px solid rgba(239,68,68,0.4);">閾值: -7.0%</div><div style="color:white; padding:10px 25px; border-radius:10px; font-size:18px; font-weight:950; border:3px solid {score_color}; background:rgba(255,255,255,0.08); box-shadow:0 0 20px {score_color}33;">{score_label}</div></div></div></div><div style="flex:2.2; background:rgba(255,255,255,0.02); padding:35px; border-radius:16px; border:1px solid rgba(56, 189, 248, 0.15);"><div style="font-size:24px; color:#38BDF8; font-weight:950; margin-bottom:30px; display:flex; align-items:center; gap:12px; border-bottom:2px solid rgba(56, 189, 248, 0.3); padding-bottom:15px;"><span style="font-size:30px;">⌛</span> 歷史反彈時程推演 (方案 A：分類精準對齊)</div><div style="display:grid; grid-template-columns:1fr; gap:10px;">{get_bar_html("📊 良性回檔模式 (< 20% DD)", norm, "#10B981")}{get_bar_html("🚨 災難崩盤模式 (>= 20% DD)", crash, "#EF4444")}</div><div style="margin-top:20px; font-size:14px; color:#64748B; font-weight:600; text-align:right; font-style:italic;">* 以上天數為「交易日」之平均值，僅供戰略參考。</div></div></div></div></div>'
    st.markdown(hud_html, unsafe_allow_html=True)
    # --- 3. 歷史分佈圖 ---

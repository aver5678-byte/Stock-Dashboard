...
def page_biz_cycle():
    ...
    scenario_onboarding_html = f"""<div style="background:linear-gradient(135deg, #1E293B 0%, #0F172A 100%); border:2px solid #334155; border-radius:12px; padding:35px; margin-bottom:80px; box-shadow:0 10px 30px rgba(0,0,0,0.3);"><h2 style="color:#F1F5F9; font-size:26px; font-weight:900; margin-top:0; margin-bottom:25px; display:flex; align-items:center; gap:12px;">📋 戰術導讀：亮燈後的資產防禦計畫</h2><div style="display:grid; grid-template-columns:1fr 1fr 1fr; gap:30px;"><div style="background:rgba(255,255,255,0.03); padding:20px; border-radius:10px; border-left:4px solid #3B82F6;"><div style="color:#7DD3FC; font-weight:800; font-size:17px; margin-bottom:12px;">🚩 階段一：極速噴發 (1-3月)</div><div style="color:#94A3B8; font-size:15px; line-height:1.7;"><b>戰術</b>：持股待漲，但禁止任何加碼。75% 的波段在此階段就會見頂，這叫「獲利最後衝刺」。</div></div><div style="background:rgba(255,255,255,0.03); padding:20px; border-radius:10px; border-left:4px solid #EF4444;"><div style="color:#FCA5A5; font-weight:800; font-size:17px; margin-bottom:12px;">⚠️ 階段二：逃命窗口 (4-6月)</div><div style="color:#94A3B8; font-size:15px; line-height:1.7;"><b>戰術</b>：強制啟動「分批獲利計畫」。撤出 50% 核心持股。此時即使沒跌，時間成本也已經極高。</div></div><div style="background:rgba(255,255,255,0.03); padding:20px; border-radius:10px; border-left:4px solid #FBBF24;"><div style="color:#FDE68A; font-weight:800; font-size:17px; margin-bottom:12px;">🧠 階段三：史詩長線 (10月+)</div><div style="color:#94A3B8; font-size:15px; line-height:1.7;"><b>戰術</b>：僅限 25% 的極端機率。若進入此階段，代表景氣大通膨，應搭配「40週乖離」同步判斷是否回調。</div></div></div></div>"""
